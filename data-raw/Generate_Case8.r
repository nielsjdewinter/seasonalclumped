# Case 8: Seasonality in d18Osw in antiphase with SST
...
d18Osw<-rnorm(length(Ty),d18Oswmean+d18Oswamp*sin(2*pi*Ty-0.5*pi),DSD)
...
Case8 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case8)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case8) - nrow(newdata)))
    }
    newdata <- cbind(Case8$column, newdata)
    # Add the new data to the storage dataframe
    Case8 <- cbind(Case8, newdata)
}
Case8$column <- NULL
colnames(Case8)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case8, file = "Case8.rda")