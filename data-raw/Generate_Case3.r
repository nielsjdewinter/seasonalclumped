# Case 3: Growth stop in warm season
...
Gstop<-28 # Set temperature for growth stop at 28 degrees
...
Case3 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case3)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case3) - nrow(newdata)))
    }
    newdata <- cbind(Case3$column, newdata)
    # Add the new data to the storage dataframe
    Case3 <- cbind(Case3, newdata)
}
Case3$column <- NULL
colnames(Case3)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case3, file = "Case3.rda")