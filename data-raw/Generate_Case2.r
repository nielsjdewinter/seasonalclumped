# Case 2: Growth stop in cold season
...
Gstop<-12 # Set temperature for growth stop at 12 degrees
GR<-rep(10/365,length(Ty)) # Set growth rate to 10 mm/yr, create daily GR vector
GR[which(SST<Gstop)]<-0 # Set GR to zero for parts of shell where growth stops
...
Case2 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case2)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case2) - nrow(newdata)))
    }
    newdata <- cbind(Case2$column, newdata)
    # Add the new data to the storage dataframe
    Case2 <- cbind(Case2, newdata)
}
Case2$column <- NULL
colnames(Case2)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case2, file = "Case2.rda")