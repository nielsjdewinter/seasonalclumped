# Case 16: Lagoonal case - Seasonal change in growth rate with slower growth in summer and linear growth decrease.
# Pulse of increased d18Osw in summer (evaporation)
...
GRmeanslope<--0.5 # Set annual reduction in growth rate (-0.5 mm/yr)
...
Case16 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case16)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case16) - nrow(newdata)))
    }
    newdata <- cbind(Case16$column, newdata)
    # Add the new data to the storage dataframe
    Case16 <- cbind(Case16, newdata)
}
Case16$column <- NULL
colnames(Case16)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case16, file = "Case16.rda")