...
Case20 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case20)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case20) - nrow(newdata)))
    }
    newdata <- cbind(Case20$column, newdata)
    # Add the new data to the storage dataframe
    Case20 <- cbind(Case20, newdata)
}
Case20$column <- NULL
colnames(Case20)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case20, file = "Case20.rda")