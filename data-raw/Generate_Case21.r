...
Case21 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case21)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case21) - nrow(newdata)))
    }
    newdata <- cbind(Case21$column, newdata)
    # Add the new data to the storage dataframe
    Case21 <- cbind(Case21, newdata)
}
Case21$column <- NULL
colnames(Case21)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case21, file = "Case21.rda")