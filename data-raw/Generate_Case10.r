# Case 10: Decrease in d18Osw (freshwater pulse) in spring, d18Osw constant rest of year
...
Case10 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case10)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case10) - nrow(newdata)))
    }
    newdata <- cbind(Case10$column, newdata)
    # Add the new data to the storage dataframe
    Case10 <- cbind(Case10, newdata)
}
Case10$column <- NULL
colnames(Case10)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case10, file = "Case10.rda")