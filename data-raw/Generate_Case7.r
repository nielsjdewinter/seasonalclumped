# Case 7: Seasonality in d18Osw in phase with SST
...
Case7 <- data.frame(column = rep(NA, sum(GR) / SR[1]))
for(i in 1:length(SR)){
    ...
    if(nrow(newdata) < nrow(Case7)){
        newdata <- rbind(newdata, matrix(NA, ncol = ncol(newdata), nrow = nrow(Case7) - nrow(newdata)))
    }
    newdata <- cbind(Case7$column, newdata)
    # Add the new data to the storage dataframe
    Case7 <- cbind(Case7, newdata)
}
Case7$column <- NULL
colnames(Case7)[seq(1, 26, 5)] <- paste("SR_", SR)
save(Case7, file = "Case7.rda")