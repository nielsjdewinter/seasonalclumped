...
optimization_seasonality <- function(d18Oc, # Sub-annually resolved d18Oc data 
    ...
    for(i in win){
        ...
        shift<-round(win[which(Pval < p)]/2,0)
        recond<-cbind(recond,c(runMean(d18O_mod,which(Pval==min(Pval,na.rm=TRUE)))[(shift+1):length(d18O_mod)],rep(NA,shift))) # Collect best moving window d18Oc data into matrix for export
        reconD<-cbind(reconD,c(runMean(D47_mod,which(Pval==min(Pval,na.rm=TRUE)))[(shift+1):length(d18O_mod)],rep(NA,shift))) # Collect best moving window D47 data into matrix for export
    }
